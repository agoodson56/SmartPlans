...
//
// Idempotent strategy:
//   - distributor_prices: rows we own are tagged distributor='3D Historical'
//     and notes='SEED:bid-corpus|...'. Delete those before re-insert.
//   - labor_standards: rows we own have source_standard='won-bid' AND
//     a SEED:bid-corpus marker in source_bid. Delete those before re-insert.
//   - winning_proposals: scoped delete by exact project_name we're about to
//     re-insert (preserves any user-added proposals).
//
// User-added rows (different distributor / different source_standard /
// different project_name) are never touched.

const SEED_MARKER = 'SEED:bid-corpus';
...
lines.push('-- Safe to re-run: scoped DELETEs precede each table.');
lines.push('');

lines.push(`-- ─── Idempotent reset (scoped to seeder-owned rows only) ───`);
lines.push(`DELETE FROM distributor_prices WHERE distributor = '3D Historical' AND notes LIKE '${SEED_MARKER}|%';`);
lines.push(`DELETE FROM labor_standards WHERE source_standard = 'won-bid' AND source_bid LIKE '${SEED_MARKER}|%';`);
const projectsToReplace = proposalRows.map((r) => sq(r[0]));
if (projectsToReplace.length > 0) {
    lines.push(`DELETE FROM winning_proposals WHERE project_name IN (${projectsToReplace.join(', ')});`);
}
lines.push('');
...
for (const r of laborRows) {
    // r[7] is `source_bid`; tag with SEED_MARKER prefix
    const tagged = [...r];
    tagged[7] = `${SEED_MARKER}|${r[7] ?? 'unknown'}`;
    lines.push(
